...
class Vocabulary:
    ...
    def __init__(self):
        self.idx_to_word = {0: UNK_TOKEN, 1: PAD_TOKEN, 2: NULL_TOKEN}
        self.word_to_idx = {UNK_TOKEN: 0, PAD_TOKEN: 1, NULL_TOKEN: 2}
        self.word_freqs = {}
    ...
    @staticmethod
    def from_data(filenames, min_freq=0, max_size=-1):
        """
        ...
        for word, freq in sorted(vocab.word_freqs.items(), key=lambda kv: kv[1], reverse=True):
            ...
            if freq >= min_freq and not (max_size > 0 and \
                    len(vocab.word_to_idx) == max_size+3) and word not in vocab.word_to_idx:
    ...
    @staticmethod
    def from_file(vocab_file, max_size=-1):
        """
        ...
        with open(vocab_file) as f:
            for line in f:
                if max_size > 0 and len(vocab.word_to_idx) == max_size+3:
                    break
